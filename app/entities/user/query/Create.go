...
func Create(ctx context.Context, user *user.User, db *sql.DB) error {
	const queryToUsers = `INSERT INTO users(id, username, display_name, current_meeting_id, rating, gender, phone_number, birthday)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
	_, err := db.Exec(queryToUsers, user.ID, user.Username, user.DisplayName, user.CurrentMeetingId, user.Rating, user.Gender, user.PhoneNumber, user.Birthday)
	if err != nil {
